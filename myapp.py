...
tes = ':green[<<<---] Выберите график'
st.title(tes)

st.subheader("""*Чаевые в ресторане*""")
st.write(tips)
...
if st.sidebar.button('**Линейный** график показывающий динамику чаевых во времени'):
    st.subheader('**Линейный** график показывающий динамику чаевых во времени')
    st.line_chart(data = sort_date, x = 'time_order' , y = 'tip')



elif st.sidebar.button('**Гистограмма** общей выручки по дате'):
    st.subheader('**Гистограмма** общей выручки по дате')
    ...
    st.subheader('**Scatterplot**, показывающий связь между total_bill и tip')    
    ...
    st.subheader('График отражающий связь между днем недели и размером счета')     
    ...
    st.subheader('**Scatter plot** с днем недели по оси **Y**, чаевыми по оси **X**')
    st.scatter_chart(data=tips, x='day', y='tip', color = 'sex')
    
elif st.sidebar.button('**Box plot** c суммой всех счетов за каждый день'):
    st.subheader('**Box plot** c суммой всех счетов за каждый день')    
    ...
    st.subheader('Scatter plot связь размера счета и чаевых, c разбивкой по курящим/некурящим.')    
